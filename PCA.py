...
class PCA_Data():
    def __init__(self, X, Y, X_more,X_train, X_test ,X_train_pca, Y_train, X_test_pca, Y_test):
        self.X, self.Y = X, Y
        self.X_more = X_more
...
def PCA():
    ...
    X, Y, df = loader.load_im('PJ1\\Face_image\\Caltech_face\\')
    X_more, Y_more, df_more = loader.load_im('PJ1\\Face_image\\more_face\\')
    le = LabelEncoder()
    Y_label = le.fit_transform(Y)
    X_train, Y_train, X_test, Y_test = split_data(X, Y_label)
    X_ =  np.concatenate((X_train, X_more), axis = 0)
    print('X.shape = ', X_.shape)
    print('Y.shape = ', Y.shape)
    # PCA
    pca = my_pca(svd_solver="randomized", n_components= 512, whiten=True)
    pca.fit_transform(X_)
    X_train_pca = pca.transform(X_train)
    X_test_pca = pca.transform(X_test)
    
    # Save data
    data = PCA_Data(X, Y, X_more, X_train , X_test ,X_train_pca, Y_train, X_test_pca, Y_test)
